...
impl Enum<'_> {
    fn validate_as_error(&self) -> Result<()> {
    ...
    fn validate_as_display(&self) -> Result<()> {
        // at the moment no validation is required, since if no #[display(\"...\")] attribute is supplied,
        // it will fallback to the default display implementation
        Ok(())
    }
    ...
    fn validate(&self) -> Result<()> {
        check_non_field_attrs(&self.attrs)?;
        match self.derive {
            DeriveType::Error => self.validate_as_error(),
            DeriveType::Display => self.validate_as_display(),
        }
    }
}
